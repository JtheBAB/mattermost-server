...
func moveChannelsCmdF(command *cobra.Command, args []string) error {
	...
	team := getTeamFromTeamArg(a, args[1])
	if team == nil {
		return errors.New("Unable to find destination team '" + args[1] + "'")
	}
	...
	channels := getChannelsFromChannelArgs(a, args[0:])
	for i, channel := range channels {
		if channel == nil {
			CommandPrintErrorln("Unable to find channel '" + args[i+0] + "'")
			continue
		}
